            ...
            <div class="box-body">
                ...
                <table class="table table-striped table-bordered table-hover" id="tabla-data" >
                    ...
                    <tbody>
                        ...
                        @endif
                            <tr>
                                <td class="font-weight-bold" style="background: rgb(231, 237, 241)"><i class="fa fa-list blue"></i><b> {{$menu["nombre"]}}</b></td>
                                @foreach($rols as $id => $nombre)
                            ...
                            @foreach($menu["submenu"] as $key => $hijo)
                                <tr>
                                    <td class="pl-20" style="background:  rgb(231, 237, 241)"><i class="fa fa-play"></i> {{ $hijo["nombre"] }}</td>
                                    @foreach($rols as $id => $nombre)
                                        <td class="text-center">
                                            <input
                                            type="checkbox"
                                ...
                                @foreach ($hijo["submenu"] as $key => $hijo2)
                                    <tr>
                                        <td class="pl-30" style="background: rgb(231, 237, 241)"><i class="fa fa-arrow-right"></i> {{$hijo2["nombre"]}}</td>
                                        @foreach($rols as $id => $nombre)
                                            <td class="text-center">
                                                <input
                                                type="checkbox"
                                    ...
                                    @foreach ($hijo2["submenu"] as $key => $hijo3)
                                        <tr>
                                            <td class="pl-40" style="background:rgb(231, 237, 241)"><i class="fa fa-arrow-right"></i> {{$hijo3["nombre"]}}</td>
                                            @foreach($rols as $id => $nombre)
                                            <td class="text-center">
                                                <input
                                                type="checkbox"
